...
func GetBit(n int64, i int32) int64 {
	return int64(uint64(n) >> uint(i)) & int64(1)
}

func SetBit(n int64, i int32) int64 {
	return n | 1 << uint(i)
}

func ClearBit(n int64, i int32) int64 {
	return n & ^(1 << uint(i))
}

func BitsToBlock64(bits int64) int32 {
	if bits % 64 == 0 {
		return int32(int64(uint64(bits) >> 6))
	}
	return int32(int64(uint64(bits >> 6) + 1))
}

func BitWidth(n int64) int32 {
	l := 1
	n = n >> 1
	for ; n != 0; n = n >> 1{
		l++
